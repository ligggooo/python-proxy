...
async def stream_handler(reader, writer, unix, lbind, protos, rserver, cipher, sslserver, debug=0, authtime=86400 * 30,
                         block=None, salgorithm='fa', verbose=DUMMY, modstat=lambda u, r, h: lambda i: DUMMY, **kwargs):
    try:
        ...
        lproto, user, host_name, port, client_connected = await proto.accept(protos, reader=reader, writer=writer,
...
async def datagram_handler(writer, data, addr, protos, urserver, block, cipher, salgorithm, verbose=DUMMY, **kwargs):
    try:
        ...
        lproto, user, host_name, port, data = proto.udp_accept(protos, data, sock=writer.get_extra_info('socket'),
                                                               **kwargs)
        if host_name == 'echo':
...
class ProxySimple(ProxyDirect):
    ...
    def logtext(self, host, port):
        return f' -> {self.rproto.name + ("+ssl" if self.sslclient else "")} {self.bind}' + self.jump.logtext(host,
                                                                                                              port)
    ...
    async def prepare_connection(self, reader_remote, writer_remote, host, port):
        reader_remote, writer_remote = proto.sslwrap(reader_remote, writer_remote, self.sslclient, False,
                                                     self.host_name)
        ...
        await self.rproto.connect(reader_remote=reader_remote, writer_remote=writer_remote, rauth=self.auth,
                                  host_name=whost, port=wport, writer_cipher_r=writer_cipher_r, myhost=self.host_name,
...
class ProxyH2(ProxySimple):
    ...
    async def handler(self, reader, writer, client_side=True, stream_handler=None, **kw):
        ...
        reader, writer = proto.sslwrap(reader, writer, self.h2sslclient if client_side else self.h2sslserver,
                                       not client_side, None)
                ...
                while len(write_buffer) > 0:
                    while conn.local_flow_control_window(stream_id) <= 0:
                        ...
                        if stream_writer.closed:
                            break
                    chunk_size = min(conn.local_flow_control_window(stream_id), len(write_buffer),
...
class ProxyQUIC(ProxySimple):
    ...
    async def wait_quic_connection(self):
        if self.handshake is not None:
            ...
            self.quic_egress_acm = aioquic.asyncio.connect(self.host_name, self.port, create_protocol=Protocol,
    ...
    async def udp_start_server(self, args):
        ...
        return await aioquic.asyncio.serve(self.host_name, self.port, configuration=self.quicserver,
                                           create_protocol=Protocol), None
...
class ProxyH3(ProxyQUIC):
    ...
    async def wait_h3_connection(self):
        if self.handshake is not None:
            ...
            import aioquic.asyncio
            self.handshake = asyncio.get_event_loop().create_future()
            self.quic_egress_acm = aioquic.asyncio.connect(self.host_name, self.port,
    ...
    def start_server(self, args, stream_handler=stream_handler):
        import aioquic.asyncio
        return aioquic.asyncio.serve(self.host_name, self.port, configuration=self.quicserver,
                                     create_protocol=self.get_protocol(True,
                                                                       functools.partial(stream_handler, **vars(self),
                                                                                         **args)))
    ...
    async def wait_ssh_connection(self, local_addr=None, family=0, tunnel=None):
        if self.sshconn is not None and not self.sshconn.cancelled():
            ...
            conn = await asyncssh.connect(host=self.host_name, port=self.port, local_addr=local_addr, family=family,
                                          ...
                                          password=password, client_keys=client_keys, keepalive_interval=60,
                                          tunnel=tunnel)
...
def proxy_by_uri(uri, jump):
    ...
    if 'quic' in rawprotos or 'h3' in protonames:
        ...
        quicserver = aioquic.quic.configuration.QuicConfiguration(is_client=False, max_stream_data=2 ** 60,
                                                                  max_data=2 ** 60, idle_timeout=SOCKET_TIMEOUT)
        quicclient = aioquic.quic.configuration.QuicConfiguration(max_stream_data=2 ** 60, max_data=2 ** 60,
    ...
    if 'direct' in protonames:
        ...
        params = dict(jump=jump, protos=protos, cipher=cipher, users=users, rule=url.query, bind=loc or urlpath,
                      host_name=host_name, port=port, unix=not loc, lbind=lbind, sslclient=sslclient,
                      sslserver=sslserver)
...
def print_server_started(option, server, print_fn):
    for s in server.sockets:
        ...
        p = laddr[1]
        f = str(s.family)
        ipversion = "ipv4" if f == "AddressFamily.AF_INET" else (
            "ipv6" if f == "AddressFamily.AF_INET6" else "ipv?")  # TODO better
...
def main(args=None):
    ...
    parser.add_argument('-l', dest='listen', default=[], action='append', type=proxies_by_uri,
                        help='tcp server uri (default: http+socks4+socks5://:8080/)')
    parser.add_argument('-r', dest='rserver', default=[], action='append', type=proxies_by_uri,
                        help='tcp remote server uri (default: direct)')
    parser.add_argument('-ul', dest='ulisten', default=[], action='append', type=proxies_by_uri,
                        help='udp server setting uri (default: none)')
    parser.add_argument('-ur', dest='urserver', default=[], action='append', type=proxies_by_uri,
    ...
    if args.pac:
        ...
        for i, option in enumerate(args.rserver):
            pactext += (
                           f'var m{i}=/^(:?{option.rule.__self__.pattern})$/i;if(m{i}.test(h))' if option.rule else '') + 'return "PROXY %(host)s";'
    ...
    def print_fn(option, bind=None):
        print('Serving on', (bind or option.bind), 'by',
    ...
    def print_fn(option, bind=None):
        print('Serving on UDP', (bind or option.bind), 'by', ",".join(i.name for i in option.protos),
    ...
    def print_fn(option, bind=None):
        print('Serving on', (bind or option.bind), 'backward by',
              ",".join(i.name for i in option.protos) + ('(SSL)' if option.sslclient else ''),
              '({}{})'.format(option.cipher.name, ' ' + ','.join(i.name() for i in
                                                                 option.cipher.plugins) if option.cipher and option.cipher.plugins else '') if option.cipher else '')
